  ...
  extensionContext.registerMessageConverter({
    ...
    create: () => {
      ...
      return (msgEvent: MessageEvent<unknown>) => {
        ...
        return convertToCartesian(message, lookupTable, msgEvent.receiveTime);
      };
    },
  });
}
...
/**
 * Converts nanoseconds to Time format
 */
function nsToTime(ns: number): Time {
  const sec = Math.floor(ns / 1e9);
  const nsec = Math.floor(ns - sec * 1e9);
  return { sec, nsec };
}
...
function convertToCartesian(
  ...
  return {
    timestamp: nsToTime(scan.timestamp_ns),
    frame_id: "ouster",
    pose: {
